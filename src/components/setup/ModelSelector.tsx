...
export function ModelSelector({ models }: ModelSelectorProps) {
  ...
  const handleStart = async (e: React.FormEvent) => {
    ...
    const newRecent = [whiteModel, blackModel, ...recentModels]
    ...
    startTransition(async () => {
      ...
      formData.append("whiteModel", whiteModel);
      formData.append("blackModel", blackModel);
  ...
  return (
    <div className="w-full max-w-6xl mx-auto flex flex-col gap-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 bg-charcoal-900/50 p-6 rounded-2xl border border-charcoal-800">
        <div 
          onClick={() => setActiveSelection("white")}
          className={`p-4 rounded-xl cursor-pointer border-2 transition-colors ${activeSelection === "white" ? "border-amber-500 bg-charcoal-800" : "border-charcoal-700 bg-charcoal-950 hover:border-charcoal-500"}`}
        >
          <div className="text-xs font-bold uppercase tracking-wider text-charcoal-400 mb-2">White Model</div>
          <div className="font-display font-bold text-xl text-amber-500">{getModelDetails(whiteModel).name}</div>
          <div className="text-sm font-mono text-charcoal-500">{whiteModel}</div>
        </div>

        <div 
          onClick={() => setActiveSelection("black")}
          className={`p-4 rounded-xl cursor-pointer border-2 transition-colors ${activeSelection === "black" ? "border-charcoal-300 bg-charcoal-800" : "border-charcoal-700 bg-charcoal-950 hover:border-charcoal-500"}`}
        >
          <div className="text-xs font-bold uppercase tracking-wider text-charcoal-400 mb-2">Black Model</div>
          <div className="font-display font-bold text-xl text-charcoal-100">{getModelDetails(blackModel).name}</div>
          <div className="text-sm font-mono text-charcoal-500">{blackModel}</div>
        </div>
      ...
      <div className="flex flex-col gap-6">
        <div className="flex items-center gap-4">
          <h2 className="text-2xl font-display font-bold text-charcoal-100">
            {activeSelection === "white" ? "Select White Model" : "Select Black Model"}
          </h2>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {displayModels.map(m => {
            const isSelected = activeSelection === "white" ? whiteModel === m.id : blackModel === m.id;
            return (
              <div 
                key={m.id}
